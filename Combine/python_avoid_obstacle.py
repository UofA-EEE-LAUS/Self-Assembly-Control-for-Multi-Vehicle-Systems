...
# which we dot from the Pioneer_p3dx manual for all the proximity sensors relative to the robots centre 
...
for x in range(1,16+1):
        ...
        sensor_val=np.append(sensor_val,np.linalg.norm(detectedPoint)) #get list of values, using the linear algebra lib from numpy to convert the three dimensional image into one dimensional number.
...
while (time.time()-t)<60:
    ...
    kp=0.5	#steering gain, proportional constanst for the steer value
    vl=v+kp*steer
    vr=v-kp*steer
    print "V_l =",vl
    print "V_r =",vr

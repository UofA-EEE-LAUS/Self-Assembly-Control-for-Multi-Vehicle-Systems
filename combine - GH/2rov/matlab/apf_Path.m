function [ point ] = apf_Path(begin,over,obstacle,rov)
%PATH_PLAN 
load('test.mat');
obstacles = [];
sx=size(x);
...
%         testOut(:,i)=apf_Algrith(testPoint(:,i),over,obstacle); % the output is the resultant
        testOut(:,i)=apf_LM(testPoint(:,i),over,obstacle,rov); % the output is the resultant
...
%               testOut_m2(:,j)=apf_LM(testPoint_m2(:,j),over,new_ob); 
                testOut_m2(:,j)=apf_LM(testPoint_m2(:,j),over,new_ob,rov); 

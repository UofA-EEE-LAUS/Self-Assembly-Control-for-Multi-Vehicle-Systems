function [ point ] = apf_Path(begin,over,obstacle,range)
%PATH_PLAN 
...
%         testOut(:,i)=apf_Algrith(testPoint(:,i),over,obstacle); % the output is the resultant
        testOut(:,i)=apf_LM(testPoint(:,i),over,obstacle,range); % the output is the resultant
...
%               testOut_m2(:,j)=apf_LM(testPoint_m2(:,j),over,new_ob); 
                testOut_m2(:,j)=apf_LM(testPoint_m2(:,j),over,new_ob,range); 

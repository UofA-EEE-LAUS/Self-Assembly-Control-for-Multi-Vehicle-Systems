function [ position ] = apfpath(begin,goal,obstacles,radius,extraradius,katt,krep,kextra,iterations,n)
%% Initiallization
count=1;
point=begin;
%% Generate Path
while (norm(point-goal)>0.2) && (count<=iterations) 
    ...
    for i=1:8 
        ...
        force(:,i)=apffield(nposition(:,i),goal,obstacles,radius,extraradius,katt,krep,kextra); 
    end
    [~, If]=min(force);
    ...
    if count > 2
        ...
        if (norm(cur-pre)) < 0.01 
            ...
            for j=1:n
                ...
                force_vo(:,j)=apffield(nposition_vo(:,j),goal,vobstacles,extraradius,katt,krep,kextra);
            end
            [~, If_vo]=min(force_vo);
